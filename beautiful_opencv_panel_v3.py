...
class BeautifulOpenCVPanelV2:
    ...
    def detect_cameras(self):
        ...
        available = []
        for i in range(4):  # Check first 4 camera indices (reduced from 10)
            cap = cv2.VideoCapture(i, cv2.CAP_DSHOW)  # CAP_DSHOW for Windows - faster
            if cap.isOpened():
    ...
    def create_controls_panel(self):
        ...
        camera_module = CollapsibleModule(controls_scroll, "📹 Camera Selection", start_open=True,
                                         fg_color=self.colors['bg'], corner_radius=10)
        camera_module.pack(fill="x", padx=5, pady=5)
        self.create_camera_controls(camera_module.content)
    ...
    def create_camera_controls(self, parent):
        ...
        camera_info = ctk.CTkLabel(
            parent,
            text=f"Current: Camera {self.current_camera_index}",
            font=("Georgia", 11),
            text_color=self.colors['text_medium']
        )
        camera_info.pack(pady=(0, 10))
        self.camera_info_label = camera_info
        
        # Available cameras info
        if len(self.available_cameras) > 1:
            available_text = f"Available: {', '.join([f'Camera {i}' for i in self.available_cameras])}"
        else:
            available_text = "Only 1 camera detected"
        ...
        for cam_index in self.available_cameras:
            btn = ctk.CTkButton(
                ...
                command=lambda idx=cam_index: self.on_camera_select(idx),
                font=("Georgia", 11),
    ...
    def on_camera_select(self, camera_index):
        """Handle camera selection"""
        self.switch_camera(camera_index)
        self.camera_info_label.configure(text=f"Current: Camera {camera_index}")
    
    def refresh_cameras(self):
        """Re-detect available cameras"""
        self.available_cameras = self.detect_cameras()
        # Refresh the camera controls display
        # Would need to rebuild the module to show new cameras
