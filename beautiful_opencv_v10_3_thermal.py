"""
Beautiful OpenCV Control Panel - Version 10.3 THERMAL
Modern, elegant GUI for multi-camera OpenCV processing with auto-capture + THERMAL CAMERA SUPPORT

NEW IN V10.3 THERMAL:
- OPTRIS PI 1M THERMAL CAMERA SUPPORT via Optris OTC SDK
- ThermalCameraFeed class for thermal camera integration
- ThermalFrameClient for thermal frame processing
- Automatic detection of both standard and thermal cameras
- False color thermal image display
- All v10.2 features: two-window design, responsive grid layout, etc.
...
# Try to import Optris OTC SDK for thermal cameras (optional)
try:
    import optris.otcsdk as otc
    HAS_THERMAL = True
    print("✓ Optris OTC SDK loaded - thermal camera support enabled")
except ImportError:
    HAS_THERMAL = False
    print("Note: Optris OTC SDK not available. Thermal camera support disabled.")
    print("      Install with: pip install optris-otcsdk")
...
class ThermalFrameClient(otc.IRImagerClient if HAS_THERMAL else object):
    """
    Thermal camera client that receives thermal frames from Optris cameras
    Based on ImagerShow from simple_view.py but adapted for integration
    """
    def __init__(self, serial_number=0):
        """Initialize thermal camera client"""
        if not HAS_THERMAL:
            raise RuntimeError("Optris OTC SDK not available")

        super().__init__()

        # Create IRImager instance
        self._imager = otc.IRImagerFactory.getInstance().create('native')
        self._imager.addClient(self)
        self._imager.connect(serial_number)

        # Create image builder for false color conversion
    ...
    def start(self):
        """Start the thermal camera thread"""
        if not self._running:
            self._running = True
            self._thread = threading.Thread(target=self._imager.run, daemon=True)
            self._thread.start()

    def stop(self):
        """Stop the thermal camera"""
        if self._running:
            self._running = False
    ...
    def get_frame(self):
        """Get the latest thermal frame as a false color BGR image"""
        with self._thermal_frame_lock:
            if not self._thermal_frame_updated or self._thermal_frame.isEmpty():
                return None

            # Convert thermal frame to false color image
    ...
    def get_device_info(self):
        """Get thermal camera device information"""
        try:
            device_type = otc.deviceTypeToString(self._imager.getDeviceType())
            serial = self._imager.getSerialNumber()
            mode = self._imager.getActiveOperationMode()
            return f"{device_type} (S/N {serial}) {mode.getFrameWidth()}x{mode.getFrameHeight()}"
        except:
    ...
    def onFlagStateChange(self, flagState):
        """Called when the shutter flag state changes"""
        with self._flag_state_lock:
            self._flag_state = flagState

    def onConnectionLost(self):
        """Called when connection to camera is lost"""
        self._running = False
...
class ThermalCameraFeed:
    """
    Manages a thermal camera feed using Optris OTC SDK
    Interface compatible with regular CameraFeed class
    """
    def __init__(self, serial_number=0, name=None):
        """Initialize thermal camera feed"""
        if not HAS_THERMAL:
            raise RuntimeError("Thermal camera support not available")

        self.camera_index = f"thermal_{serial_number}"
        self.serial_number = serial_number
        self.name = name or f"Thermal Camera {serial_number}"

        self.client = None
        self.is_active = False
        self.last_frame = None

        # Zoom and pan (compatible with regular CameraFeed)
        self.zoom_level = 1.0
        self.pan_x = 0
        self.pan_y = 0
    ...
    def open(self):
        ...
        try:
            if self.client is None:
                # Initialize Optris SDK if not already done
                try:
                    otc.Sdk.init(otc.Verbosity_Info, otc.Verbosity_Off, "ThermalCamera")
                    # Add Ethernet detector for network cameras
                    otc.EnumerationManager.getInstance().addEthernetDetector("192.168.0.0/24")
                except:
                    pass  # SDK already initialized

                self.client = ThermalFrameClient(self.serial_number)
                self.client.start()

                # Update name with device info
                self.name = self.client.get_device_info()
                self.is_active = True
            return self.is_active
        except Exception as e:
            print(f"Failed to open thermal camera: {e}")
            self.is_active = False
            return False
    ...
    def read(self):
        ...
        if self.client and self.is_active:
            ...
            if frame is not None:
                ...
                # Apply zoom and pan if needed (same as regular camera)
                if self.zoom_level > 1.0:
                    frame = self.apply_zoom_pan(frame)
    ...
    def apply_zoom_pan(self, frame):
        """Apply zoom and pan to frame (same as CameraFeed)"""
        h, w = frame.shape[:2]
...
class BeautifulOpenCVPanelV10_3_Thermal(TkinterDnD.Tk if HAS_DND else ctk.CTk):
    def __init__(self):
        super().__init__()
        ...
        # Feed window setup
        self.title("Vision Laboratory v10.3 THERMAL - Feed")
    ...
    def setup_control_window(self):
        ...
        self.control_window.title("Controls - Vision Lab v10.3 THERMAL")
    ...
    def detect_cameras(self):
        ...
        # Try to detect thermal cameras (if SDK available)
        if HAS_THERMAL:
            try:
                # Try serial number 0 (first thermal camera)
                thermal_camera = ThermalCameraFeed(serial_number=0, name="Optris Thermal Camera")
                if thermal_camera.open():
                    thermal_idx = "thermal_0"
                    self.cameras[thermal_idx] = thermal_camera
                    detected_count += 1

                    # Add checkbox for thermal camera
                    checkbox = ctk.CTkCheckBox(
                        self.camera_list_frame,
                        text=f"🌡️ {thermal_camera.name}",
                        command=lambda idx=thermal_idx: self.toggle_camera(idx),
                        font=("Georgia", 11, "bold"),
                        text_color=self.colors['text_dark'],
                        fg_color=self.colors['warning'],  # Orange for thermal
                        hover_color="#FF6347"
                    )
                    checkbox.pack(anchor="w", padx=10, pady=5)
                    self.camera_checkboxes[thermal_idx] = checkbox
                else:
                    thermal_camera.release()
            except Exception as e:
                print(f"Thermal camera detection failed: {e}")
...
if __name__ == "__main__":
    app = BeautifulOpenCVPanelV10_3_Thermal()
    app.mainloop()
