...
class BeautifulOpenCVPanelV6(TkinterDnD.Tk if HAS_DND else ctk.CTk):
    def __init__(self):
        super().__init__()
        ...
        if self.show_contours.get() or self.measure_width.get():
            ...
            if self.measure_width.get() and len(filtered_contours) > 0:
                ...
                if self.kalman_enabled.get():
                    self.kalman_filter.correct(np.array([[np.float32(measurement)]]))
                    prediction = self.kalman_filter.predict()
                    measurement = int(prediction[0][0])
                
                self.measured_width = measurement
                self.measured_width_mm = measurement / self.pixels_per_mm.get()
                
                # Update measurement display
                self.measurement_value_label.configure(
                    text=f"{self.measured_width_mm:.2f} mm ({measurement} px) {dim_label}"
                )
