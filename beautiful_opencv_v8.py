"""
Beautiful OpenCV Control Panel - Version 8.0
Modern, elegant GUI for multi-camera OpenCV processing

NEW IN V8:
- Measure ALL filtered contours (multiple molten zones simultaneously)
- Dual measurement display: on-screen labels + numbered list
- Adjustable morphological closing iterations
- Capture mode selector: Live / 3fps / 1fps / 1 per 5sec
- Frame Grab (save processed frame) vs Screenshot (capture entire window)
- Division by zero protection for calibration
- HSV color filtering for molten germanium detection
- Width vs Height measurement toggle
...
class BeautifulOpenCVPanelV8(TkinterDnD.Tk if HAS_DND else ctk.CTk):
    def __init__(self):
        super().__init__()
        
        self.title("OpenCV Vision Laboratory v8.0 🔬✨")
        ...
        # Capture mode
        self.capture_mode = ctk.StringVar(value="1fps")  # Live, 3fps, 1fps, 5sec
    ...
    def take_screenshot(self):
        """Capture the entire program window"""
        timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
        ...
        self.status_label.configure(
            text=f"🖼️ Screenshot saved: {filename}",
            text_color=self.colors['success']
        )
    ...
    def update_frame(self):
        """Capture and display video with adjustable capture rate"""
        import time
